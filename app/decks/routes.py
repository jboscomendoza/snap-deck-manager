...
PATH_CARDS_JSON = path.join("app", "marvel-snap-cards", "marvel-snap-cards.json")
...
def index():
    title = "Summary"
    deck_count = db.session.execute(
        db.func.count(Deck.id)
    ).scalar()
...
def add_decks():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        return redirect(url_for("decks.get_decks"))
    return render_template(
        "add-decks.html",
        title=title,
        form=form,
...
def delete_deck(id: int):
    ...
    deleted_deck = db.first_or_404(db.select(Deck).filter_by(id=id))
    if request.method == "POST" and request.form["confirm"] == "Yes":
        db.session.execute(db.delete(Deck).filter_by(id=deleted_deck.id))
        db.session.commit()
...
def get_tags(tag=None):
    if not tag:
        ...
        tags = db.session.execute(
            db.select(Tag.name, db.func.count(Deck.id))
            .join(Deck.tags)
            .distinct()
            .group_by(Tag.id)
        )
        ...
        decks = (
            db.session.execute(db.select(Deck).join(Deck.tags).filter(Tag.name == tag))
            .scalars()
            .unique()
        )
        return render_template(
...
def view_deck(id):
    deck = db.first_or_404(db.select(Deck).filter_by(id=id))
    ...
    if edit and request.method == "POST":
        ...
        for tag_name in tag_names:
            ...
            if not tag_in_db:
                new_tag = Tag(name=tag_name)
                deck.tags.append(new_tag)
            elif tag_in_db and (tag_in_db not in deck.tags):
                deck.tags.append(tag_in_db)
